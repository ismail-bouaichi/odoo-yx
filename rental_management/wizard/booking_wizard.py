...
class BookingWizard(models.TransientModel):
    ...
    is_parking = fields.Selection(selection=[('0', 'Non'), ('1', 'Oui')], string="Parking", required=True)
    reduction = fields.Float(string="Montant Reduction")
    source_reduction = fields.Selection(selection=[
        ('office', 'Bureau'),
        ('administration', 'Administration'), ],
        default='office', string="Source Reduction")
    ...
    def create_booking_action(self):
        ...
        if self.book_price < 100000:
            raise UserError(_(" le montant de l'avance, ne doit pas être inférieur à 100 000,00 MAD."))
        ...
        self._context.get('from_crm')
        data = {
            'customer_id': self.customer_id.id,
            'property_id': self.property_id.id,
            'book_price': self.book_price * (-1),
            'ask_price': self.ask_price if self.is_any_broker == '0' else self.ask_price + 60000.0,
        ...
        pre_sale_ids = self.env['property.presale'].search(
            [('property_id', '=', self.property_id.id), ('state', '=', 'active')])
        for pre_sale_id in pre_sale_ids:
            pre_sale_id.state = 'converted'
